...
const PM25 = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (response.data && response.data.pm25 !== undefined) {
                    setPm25Value(response.data.pm25);
                } else {
                    console.warn('PM2.5 data not available in the response');
                }
            } catch (error) {
    ...
    return (
        <Box sx={{
            ...
            <Typography variant="h5" sx={{ marginBottom: 1 }}>PM2.5</Typography>
            <div style={{ width: 70, height: 70 }}>
                <CircularProgressbar
                    // value={pm25Value || 0}
                    value={(pm25Value / 250.5) * 100}
                    text={`${pm25Value !== null ? pm25Value : 0} μg/m³`}
                    styles={buildStyles({
                        pathColor: color,
                        textColor: '#000',
                        trailColor: '#d6d6d6',
